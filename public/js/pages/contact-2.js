  ...
  (function setupQuestionnaire() {
    ...
    try {
      ...
      console.error(
        "[contact] Données pôles introuvables ou invalides (#jcd-poles-data)",
        e,
      );
    }
    ...
    function recapHTML() {
      ...
      return ids
        .map(function (id) {
          var p = POLE_BY_ID[id];
          return (
            '<span class="recap-chip" style="--pc: var(--c-' +
            p.id +
            ')">' +
            esc(p.name) +
            "</span>"
    ...
    function applyURLParams() {
      ...
      if (intentParam === "assistance") {
        ...
        if (msg)
          msg.value =
            "Bonjour, je souhaite rejoindre le Groupe JCD (candidature spontanée). ";
      }
      updateCount();
    ...
    function buildDetails(ids) {
      ...
      lines.push(
        "Type de demande : " +
          (currentIntent === "assistance"
            ? "Assistance (client existant)"
            : "Nouveau projet / question"),
      );
      lines.push(
        "Pôles sollicités : " +
          (ids.length
            ? ids
                .map(function (id) {
                  return POLE_BY_ID[id].name;
                })
                .join(", ")
            : "Non précisé (demande générale)"),
      );
      return lines.join("\n");
    }
    function buildSubject(ids, company, name) {
      var tags = ids
        .map(function (id) {
          return "[" + POLE_BY_ID[id].name.toUpperCase() + "]";
        })
        .join("");
      var who =
      ...
      return (
        (tags ? tags + " " : "") +
        who +
        (currentIntent === "assistance" ? " — Assistance" : " — Contact")
      );
    }
        ...
        sp.textContent =
          ids.length === 1
            ...
            : ids
                ...
                .join(", ") +
              " et " +
              POLE_BY_ID[ids[ids.length - 1]].name;
      }
