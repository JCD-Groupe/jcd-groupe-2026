  ...
  (function setupCarrieres() {
    ...
    var POLE_COLORS = {
      ...
      print: '#F3941D', telecom: '#B063AC', formation: '#8EC041', agencement: '#A78D75'
    };
    /* ------------------------------------------------------------------ */

    var OFFRE_BY_ID = {};
    OFFRES.forEach(function (o) { OFFRE_BY_ID[o.id] = o; });
    ...
    function renderOffers() {
      if (!listEl) return;
      if (OFFRES.length === 0) {
        listEl.innerHTML = '<div class="offers-empty" role="listitem">Aucune offre ouverte pour le moment, '
          + 'les candidatures spontanées restent les bienvenues.</div>';
        return;
      }
      listEl.innerHTML = OFFRES.map(function (o) {
        return '<article class="offer-card" role="listitem"'
          + ' style="--pc: ' + (POLE_COLORS[o.pole] || 'var(--accent)') + '">'
          + '<span class="offer-pole">' + esc(o.poleName) + '</span>'
          + '<h3 class="offer-title">' + esc(o.titre) + '</h3>'
          + '<span class="offer-meta">'
          +   '<span><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>' + esc(o.lieu) + '</span>'
          +   '<span class="meta-sep" aria-hidden="true"></span>'
          +   '<span>' + esc(o.contrat) + '</span>'
          +   '<span class="meta-sep" aria-hidden="true"></span>'
          +   '<span>' + esc(o.temps) + '</span>'
          + '</span>'
          ...
          + '</button>'
          + '<div class="offer-desc"><div><p>' + esc(o.desc || '') + '</p></div></div>'
          + '<div class="offer-foot">'
          +   '<a class="offer-apply" href="#postuler" data-offre="' + esc(o.id) + '"'
          +     ' aria-label="Postuler — ' + esc(o.titre) + ', ' + esc(o.lieu) + ', ' + esc(o.contrat) + '">Postuler'
          +     '<span class="offer-apply-arrow" aria-hidden="true">' + ARROW + '</span>'
          +   '</a>'
          + '</div>'
          + '</article>';
      }).join('');
    ...
    function setupOffersCarousel() {
      ...
      prev.addEventListener('click', function () {
        listEl.scrollBy({ left: -step(), behavior: prefersReduced ? 'auto' : 'smooth' });
      });
    ...
    function renderHeroCount() {
      ...
      heroCount.textContent = n === 0
        ...
        : n + (n === 1 ? ' poste ouvert' : ' postes ouverts') + ' à Metz & Nancy';
    }
    ...
    document.querySelectorAll('[data-intent-link="spontanee"]').forEach(function (a) {
      a.addEventListener('click', function () { setIntent('spontanee'); });
    });
    ...
    function setFile(file) {
      ...
      if (CV_EXTENSIONS.indexOf(ext) === -1) {
        fileInput.value = '';
        cvError('Format non pris en charge : merci de joindre un PDF, DOC ou DOCX.');
        return;
      }
      if (file.size > MAX_CV_BYTES) {
        fileInput.value = '';
        cvError('Fichier trop lourd (' + fmtSize(file.size) + ') : la limite est de 5 Mo.');
        return;
      }
      errorEl.hidden = true;
    ...
    drop.addEventListener('drop', function (e) {
      ...
      if (files && files.length) {
        ...
        } catch (err) { /* anciens navigateurs : on garde la sélection manuelle */ }
        setFile(files[0]);
      }
    });
    ...
    function setFieldError(id, on) {
      ...
      if (field && field.closest('.q-field')) field.closest('.q-field').classList.toggle('has-error', on);
    }
    function validate() {
      ...
      if (!fnOk) { problems.push('votre prénom'); firstBad = firstBad || firstname; }
      if (!lnOk) { problems.push('votre nom'); firstBad = firstBad || lastname; }
      if (!phOk) { problems.push('un numéro de téléphone valide'); firstBad = firstBad || phone; }
      if (!emOk) { problems.push('un email valide'); firstBad = firstBad || email; }

      if (currentIntent === 'offre') {
        var ofOk = !!selOffre.value;
        setFieldError('f-offre', !ofOk);
        if (!ofOk) { problems.push('l’offre concernée'); firstBad = firstBad || selOffre; }
      } else {
        var poOk = inpPoste.value.trim().length > 0;
        setFieldError('f-poste', !poOk);
        if (!poOk) { problems.push('le poste recherché'); firstBad = firstBad || inpPoste; }
      }

      var cvOk = !!(fileInput.files && fileInput.files.length);
      drop.classList.toggle('has-error', !cvOk);
      if (!cvOk) { problems.push('votre CV'); firstBad = firstBad || fileInput; }
    ...
    function buildSubject() {
      var who = document.getElementById('f-firstname').value.trim() + ' ' +
                document.getElementById('f-lastname').value.trim();
      return (currentIntent === 'spontanee' ? '[CANDIDATURE SPONTANÉE] ' : '[CANDIDATURE OFFRE] ')
        + postLabel() + ' — ' + who;
    }
    ...
    function showSuccess() {
      ...
      if (recap) {
        recap.textContent = (currentIntent === 'spontanee' ? 'Candidature spontanée : ' : '') + postLabel();
      }
    ...
    form.addEventListener('submit', function (e) {
      ...
      if (!accessKey || accessKey.indexOf('VOTRE_CLE') === 0) {
        errorEl.textContent = 'Le formulaire n’est pas encore relié à Web3Forms (clé d’accès manquante). En attendant, envoyez votre CV à recrutement@jcd-groupe.fr ou appelez le 03 87 18 49 20.';
        errorEl.hidden = false;
        return;
      }
      ...
      fd.append('botcheck', document.getElementById('f-botcheck').checked ? 'on' : '');
      fd.append('type_candidature', currentIntent === 'spontanee' ? 'Candidature spontanée' : 'Réponse à une offre');
      ...
      fetch('https://api.web3forms.com/submit', {
        ...
        .then(function (json) {
          ...
          if (json && json.success) {
            ...
            errorEl.textContent = (json && json.message) ? json.message : 'L’envoi a échoué. Réessayez ou écrivez à recrutement@jcd-groupe.fr.';
            errorEl.hidden = false;
          }
        })
        .catch(function () {
          setLoading(false);
          errorEl.textContent = 'Connexion impossible. Réessayez ou écrivez à recrutement@jcd-groupe.fr.';
          errorEl.hidden = false;
        });
    });
