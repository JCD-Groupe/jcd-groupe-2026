...
(function setupProgramAccordion() {
  ...
  function syncExpandAll() {
    if (!all) return;
    var everyOpen = mods.every(function (m) {
      return m.getAttribute("data-open") === "true";
    });
...
(function setupTocSpy() {
  var links = Array.prototype.slice.call(
    document.querySelectorAll(".fd-toc-item"),
  );
  if (!links.length || !("IntersectionObserver" in window)) return;
  var map = {};
  links.forEach(function (l) {
    map[l.getAttribute("data-spy")] = l;
  });
  var io = new IntersectionObserver(
    function (entries) {
...
function applyTosaVisibility(catSlug) {
  ...
  document.querySelectorAll("[data-tosa]").forEach(function (n) {
    n.remove();
  });
}
...
(function hydrateFromCatalogue() {
  ...
  if (!f || !index[f]) {
    applyTosaVisibility(cat || "bureautique");
    return;
  }
  var entries = index[f];
  var e = null;
  ...
  document.querySelectorAll('[data-field="title"]').forEach(function (n) {
    n.textContent = e.t;
  });
  ...
  if (f !== "excel-initiation") {
    document.querySelectorAll('[data-field="reference"]').forEach(function (n) {
      n.textContent = "—";
    });
  }
...
(function setupCatalogueMenu() {
  ...
  function getCatSegment(path) {
    var segments = path.split("/").filter(Boolean); // filter(Boolean) retire les chaines vides (ex: slash final)
    return segments.length >= 2
      ? segments[segments.length - 2].toLowerCase()
      : "";
  }
  var here = getCatSegment(location.pathname);
  cats.forEach(function (c) {
